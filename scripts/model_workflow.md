...
flowchart TD
    ...
    subgraph ANALYSIS["Post-solve analysis — geocanoe.analysis"]
        ...
        X3["network.py<br/>Contract deployed routes and calculate centrality"]
    end
    ...
    X3 --> F3["analysis_outputs/<source-run>/network/<br/>Centrality CSVs, GraphML, manifest"]
```
...
Editable or regular installation through `pyproject.toml` provides eleven
canonical commands:
...
geocanoe-network        → geocanoe.analysis.network:main
geocanoe-network-plot   → geocanoe.analysis.network_viz:main
geocanoe-network-html   → geocanoe.analysis.network_html:main
```
...
- `geocanoe.analysis.network` decodes solved pseudo-edge flows, contracts
  pass-through chains, and exports centrality tables and NetworkX GraphML files.
- `geocanoe.analysis.network_viz` renders projected GraphML networks with
  configurable centrality sizing, community/role colouring, and graph layouts.
- `geocanoe.analysis.network_html` adapts TEMOA's Gravis/D3 renderer to positive
  optimized region-commodity flows and contracted regional corridors.
...
```bash
python -m geocanoe.analysis.network --period 2025
```

Visualize a completed network analysis:

```bash
geocanoe-network-plot
```

Create a draggable, zoomable TEMOA-style HTML network:
...
- Post-solve graph metrics are generated by `geocanoe.analysis.network`.
