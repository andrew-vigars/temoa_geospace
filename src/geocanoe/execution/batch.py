"""
Run ordered batches of CANOE/TEMOA model configurations.

This module loads a Geospatial-CANOE batch configuration, validates each
enabled TEMOA run configuration and its referenced SQLite database, and
executes model runs sequentially through the package-native single-run
execution workflow.

Each model run is launched as an independent subprocess. A failed run does not
terminate the remaining batch unless ``continue_on_failure`` is disabled.
Batch status is persisted incrementally to JSON and CSV so completed work
remains recorded if execution is interrupted.

Inputs
------
config/batch_profiles/*.toml
...
- Individual model-run outputs are managed by ``geocanoe.execution.run``.
"""
...
def build_main_run_command(run: BatchRun) -> list[str]:
    """Build the package-native single-run command for one batch entry."""
