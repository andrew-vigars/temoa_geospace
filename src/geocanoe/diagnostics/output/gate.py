...
from geocanoe.diagnostics.models import (
    DiagnosticReport,
    DiagnosticResult,
    DiagnosticStage,
)
...
def read_table(con: sqlite3.Connection, table: str) -> pd.DataFrame:
    """Read a required SQLite table or raise a validation error."""
...
def read_optional_table(con: sqlite3.Connection, table: str) -> pd.DataFrame:
    """Read an optional SQLite table, returning an empty DataFrame if absent."""
...
def split_edge_region(region: str) -> tuple[str, str]:
    """Split an edge pseudo-region into origin and destination regions."""
...
def within_tolerance(
    ...
    """Compare two values using combined absolute and relative tolerances."""
...
def summarize_failures(df: pd.DataFrame, sort_col: str, max_rows: int) -> pd.DataFrame:
    """Return the largest absolute failures for bounded console output."""
...
def fmt_set(values: set[str]) -> str:
    """Format a set deterministically for console output."""
...
def print_check(name: str, passed: bool, detail: str = "") -> None:
    """Print one compact pass/fail result."""
...
def check_commodity_balance(
    ...
    """Evaluate conserved node commodity balances against tolerances."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    objective_comparison = pd.DataFrame()
    report = DiagnosticReport(metadata={"database": str(db_path)})
    ...
    try:
        ...
        report.add(
            DiagnosticResult(
                ...
                severity="ERROR",
                detail=(
                    f"checked={len(balance)}, excluded={len(balance_excluded)}, "
                    f"failures={len(balance_failures)}"
                ),
                failures=balance_failures if not balance_failures.empty else None,
                check_id="OUTPUT.BALANCE.RESIDUAL",
                stage=DiagnosticStage.OUTPUT,
                remediation="Inspect commodity production, use, imports, exports, and demand at failing nodes.",
            )
        )
    ...
    try:
        ...
        report.add(
            DiagnosticResult(
                name="Edge technology diagnostics",
                passed=False,
                severity="ERROR",
                detail=str(exc),
                check_id="OUTPUT.CAPACITY.EDGE_FLOW_WITHOUT_CAPACITY",
