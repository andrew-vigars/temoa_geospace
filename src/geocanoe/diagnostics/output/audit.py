"""Detailed post-solve audit tables, plots, and summary reports."""
...
from geocanoe.diagnostics.output.gate import resolve_database_path, table_exists
...
def select_db() -> Path:
    """Interactively select one solved database from the run archive."""

    dbs = sorted(OUTPUT_ROOT.glob("**/solved_*.sqlite"))

    if not dbs:
        raise FileNotFoundError(f"No solved_*.sqlite files found under {OUTPUT_ROOT}")
...
def table_cols(con, table):
    """Return ordered column names for an existing SQLite table."""
...
def safe_ident(name):
    """Quote an SQLite identifier after escaping embedded quotes."""
...
def make_audit_dir(db_path: Path):
    """Create a collision-resistant audit directory for one database."""
...
def discover_cost_columns(con):
    """Return recognized raw and discounted OutputCost component columns."""
...
def discover_exchange_techs(con):
    """Discover technologies represented on edge pseudo-regions."""
...
def discover_etl_techs(con):
    """Discover technologies or groups represented by ETL segments."""
...
def audit_table_counts(con, out_dir):
    """Write and return row counts for every SQLite table."""
...
def audit_schema_columns(con, out_dir):
    """Write and return a complete table-column inventory."""
...
def audit_output_tables(con, out_dir):
    """Write and return an inventory of solved output tables."""
...
def audit_outputcost(con, out_dir):
    """Write technology, region-type, null, and negative cost summaries."""
...
def audit_etl_costs(con, out_dir):
    """Write OutputCost summaries for ETL-defined technologies."""
...
def audit_exchange_flows(con, out_dir):
    """Write positive edge-flow and endpoint-degree summaries."""
...
def audit_network_metrics(out_dir):
    """Build optional NetworkX metrics from positive edge flows."""
...
def plot_cost_component(con, out_dir, component):
    """Plot the largest technology totals for one cost component."""
...
def plot_total_cost_by_tech(con, out_dir):
    """Plot the largest undiscounted technology cost totals."""
...
def plot_exchange_degree(out_dir):
    """Plot edge technologies and regions with the highest endpoint degree."""
...
def plot_network_weighted_degree(out_dir):
    """Plot exchange endpoints with the highest flow-weighted degree."""
...
def write_run_report(con, db_path, out_dir):
    """Write a human-readable Markdown and text audit summary."""
...
def parse_args() -> argparse.Namespace:
    """Parse an optional database and audit output directory."""

    parser = argparse.ArgumentParser(
        description="Generate detailed post-solve audit tables and plots."
    )
    parser.add_argument(
        "database",
        nargs="?",
        type=Path,
        help=(
            "Solved SQLite database or run directory. If omitted, select an "
            "available solved database interactively."
        ),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Explicit audit output directory. Defaults to audit_outputs/<run>/<timestamp>.",
    )
    return parser.parse_args()


def main() -> int:
    """Run the detailed post-solve audit and return a CLI exit code."""
