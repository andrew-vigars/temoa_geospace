"""
Build a CANOE/TEMOA-compatible SQLite database from Geospatial-CANOE products.

This module is the schema-encoding endpoint of the Geospatial-CANOE data
pipeline. It combines a selected geospatial graph, road-connectivity product,
processed point inputs, technology parameters, and infrastructure cost models
into a CANOE/TEMOA-compatible SQLite database.

The workflow resolves inputs from a validated geospatial build configuration,
constructs canonical node and edge regions, assigns spatially distributed model
inputs to graph nodes, rebuilds topology-dependent CANOE/TEMOA tables, validates
the encoded database, and exports the completed SQLite file.
...
def validate_clean_emissions(co2_raw: gpd.GeoDataFrame) -> None:
    ...
    This function checks that the clean emissions GeoPackage produced by the
    emissions preprocessing stage satisfies the assumptions required by the
    schema builder. The layer must contain required facility, coordinate,
    emissions, and spatial-assignability fields; use WGS84; contain numeric
    latitude, longitude, and emissions values; include only spatially assignable
    records; and fall within broad Canada coordinate bounds.
    ...
    if missing_columns:
        raise ValueError(
            "Clean emissions file is missing required columns from the emissions preprocessing stage: "
    ...
    if not co2_raw["is_spatially_assignable"].astype(bool).all():
        raise ValueError(
            ...
            "Rerun the emissions preprocessing stage and verify the GPKG export filter."
        )
...
def filter_to_configured_provinces(
    ...
    if "province" not in data.columns:
        raise ValueError(
            ...
            "Run the legacy-input preprocessing stage before schema encoding."
        )
